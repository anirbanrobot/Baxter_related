...
if len(argv) < 2:
	...
	# Node initiation
	rospy.init_node('Go_to_angles_'+ argv[1])
	# Object of Limb class
	limb = baxter_interface.Limb(argv[1])
...
if len(argv) < 2:
	...
	# reassign desired joint angles
	angles['left_s0'] = float(argv[2])
	angles['left_s1'] = float(argv[3])
	angles['left_e0'] = float(argv[4])
	angles['left_e1'] = float(argv[5])
	angles['left_w0'] = float(argv[6])
	angles['left_w1'] = float(argv[7])
	angles['left_w2'] = float(argv[8])
